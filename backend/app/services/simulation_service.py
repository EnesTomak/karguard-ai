...
def run_simulation(product: SKUProfitability, req: SimulationRequest) -> SimulationResult:
    ...
    # New quantities
    new_quantity = int(current_quantity * (1 + demand_change))
    new_ad = current_ad * (1 + ad_change)
    ...
    shipping_per_unit = product.shipping_cost / product.quantity_sold if product.quantity_sold > 0 else 0
